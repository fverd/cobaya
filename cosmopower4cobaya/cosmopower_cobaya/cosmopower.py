"""
.. module:: cosmopower_cobaya.cosmopower

:Synopsis: Simple standalone CosmoPower theory wrapper for Cobaya (copied from the soliket/cosmopower directory). No need for soliket installation with this wrapper.
...
After installing cosmopower, you can use the ``CosmoPower`` theory
codes by adding the ``cosmopower_cobaya.CosmoPower`` code as a block in your parameter
files.
...
With these and with ``cosmopower_cobaya.CosmoPower`` installed and visible to cobaya, you
...
Running this with cobaya will use ``cosmopower_cobaya.CosmoPower`` as a theory to
...
The code will automatically use the correct conversion prefactors
:math:`\ell (\ell + 1) / 2 \pi` terms and similar, as well as the CMB
temperature. See the :func:`~cosmopower_cobaya.cosmopower.CosmoPower.ell_factor` and
:func:`~cosmopower_cobaya.cosmopower.CosmoPower.cmb_unit_factor` functions for more
information on how the code infers these values.
"""
import os, sys
